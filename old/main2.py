...
class MyRL():
    ...
    def compute_reward(self, action, label):
        ...
        # 定义K类和M类
        # k_class_labels = [2, 3, 5, 6, 8]  # K类标签
        # m_class_labels = [1, 4, 7]        # M类标签
        
        imp_factor=1
        # 预测完全正确的情况
        if action == label:
            reward = weight * self.reward_multiplier  # 正确分类奖励，乘以权重和倍率
        # 预测类别错误但类型正确（K类内或M类内错误）
        # elif (label in k_class_labels and action in k_class_labels) or (label in m_class_labels and action in m_class_labels):
        #     reward = 0  # 类别内错误，奖励为0
        #     imp_factor = 0  
        # 预测类别类型错误（K类与M类混淆，或与正常类混淆）
        else:
            reward = -weight * self.reward_multiplier  # 类别错误惩罚
            imp_factor = 0
    ...
    def replay_experience(self):
        ...
        states, actions, rewards, next_states, imp_factors = zip(*batch)
        ...
        imp_factors = torch.tensor(imp_factors, dtype=torch.float32, device=self.device).unsqueeze(1)
        ...
        with torch.no_grad():
           ...
           target_q = rewards + self.discount_factor * next_q * imp_factors
    ...
    def train(self, dataset):
        ...
        while self.step_count < self.t_max:
            ...
            for t in range(len(shuffled_data) - 1):
                ...
                # 计算奖励和重要性因子 (r_t, imp_factor_t = STEP(a_t, l_t))
                reward, imp_factor = self.compute_reward(action, current_label)
                ...
                # 每100步跳出，打乱一次数据
                if self.step_count >= self.batch_size and self.step_count % 100 == 0:
                    break
                # 设置当前状态为下一状态，继续循环
